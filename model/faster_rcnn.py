...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1.):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        roi_cls_locs, roi_scores, roi_depth, roi_y_rot = self.head(
            h, rois, roi_indices)
        return roi_cls_locs, roi_scores, rois, roi_indices, roi_depth, roi_y_rot
...
#         y_rot = np.concatenate(y_rot, axis=0).astype(np.float32)
    ...
    def predict(self, imgs, sizes=None, visualize=False):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            roi_cls_loc, roi_scores, rois, _,roi_depths, roi_y_rots = self(img, scale=scale)
            # We are assuming that batch size is 1.
            roi_score = roi_scores.data
            roi_cls_loc = roi_cls_loc.data
            roi = at.totensor(rois) / scale
            roi_depth = roi_depths.data
            roi_y_rot = roi_y_rots.data
            ...
            raw_depth = at.tonumpy(roi_depth)
            raw_y_rot = at.tonumpy(roi_y_rot)

            bbox, label, score, depth, y_rot = self._suppress(raw_cls_bbox, raw_prob, raw_depth, raw_y_rot)
            bboxes.append(bbox)
            labels.append(label)
            scores.append(score)
            depths.append(depth)
        ...
        self.use_preset('evaluate')
        self.train()
        return bboxes, labels, scores, depths, y_rots
